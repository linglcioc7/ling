...
case $input in
    [yY][eE][sS]|[yY])
		...
		docker pull dmtc/zhan-shen:user-login

		echo "开始安装"

		echo "命令内容："
		echo "docker run -d --net=host ${restart} --name=${name} -v ${aria2}:/downloads/aria2 -v ${utorrent}:/downloads/utorrent -v ${transmission}:/downloads/transmission -v ${qbittorrent}:/downloads/qbittorrent dmtc/zhan-shen:user-login /bin/bash ./auto.sh"
		echo ""
		echo ""

		docker run -d --net=host ${restart} --name=${name} -v ${aria2}:/downloads/aria2 -v ${utorrent}:/downloads/utorrent -v ${transmission}:/downloads/transmission -v ${qbittorrent}:/downloads/qbittorrent dmtc/zhan-shen:user-login /bin/bash ./auto.sh
